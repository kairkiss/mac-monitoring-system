...
final class TelegramService: ObservableObject {
    ...
    func sendVideo(videoData: Data, caption: String = "", fileName: String? = nil) {
        sendVideoWithRetry(videoData: videoData, caption: caption, attempt: 0, fileName: fileName)
    }
    ...
    private func sendVideoWithRetry(videoData: Data, caption: String, attempt: Int, fileName: String? = nil) {
        let settings = SettingsStore.shared
        let token = settings.telegramBotToken
        let chatID = settings.telegramChatID
        ...
        if attempt == 0 {
            ActivityLogManager.shared.log(level: .info, category: .telegram, message: "Telegram video send started", relatedFile: fileName)
            DispatchQueue.main.async { [weak self] in
                self?.isSending = true
                self?.lastSendStatus = nil
            }
        ...
        let boundary = "Boundary-\(UUID().uuidString)"
        let url = URL(string: "https://api.telegram.org/bot\(token)/sendVideo")!
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.setValue("multipart/form-data; boundary=\(boundary)", forHTTPHeaderField: "Content-Type")
        ...
        let videoName = fileName ?? "video.mp4"
        body.append("--\(boundary)\r\n".data(using: .utf8)!)
        body.append("Content-Disposition: form-data; name=\"video\"; filename=\"\(videoName)\"\r\n".data(using: .utf8)!)
        ...
        URLSession.shared.dataTask(with: request) { [weak self] data, response, error in
            ...
            if let error {
                if attempt < self.maxRetries {
                    DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
                        self.sendVideoWithRetry(videoData: videoData, caption: caption, attempt: attempt + 1, fileName: fileName)
                    }
                    return
                }
                ActivityLogManager.shared.log(level: .error, category: .telegram, message: "Telegram video send failed", detail: error.localizedDescription, relatedFile: fileName)
                DispatchQueue.main.async {
                    self.isSending = false
                    self.lastSendStatus = .failure(error.localizedDescription)
                }
            ...
            if let httpResponse = response as? HTTPURLResponse {
                if httpResponse.statusCode == 200 {
                    ActivityLogManager.shared.log(level: .success, category: .telegram, message: "Telegram video send succeeded", relatedFile: fileName)
                    if let fileName {
                        MediaIndexStore.shared.markTelegramSent(fileName)
                    }
                    DispatchQueue.main.async {
                    ...
                    let responseBody = data.flatMap { String(data: $0, encoding: .utf8) } ?? "Unknown error"
                    ActivityLogManager.shared.log(level: .error, category: .telegram, message: "Telegram video send failed: HTTP \(httpResponse.statusCode)", detail: responseBody, relatedFile: fileName)
                    DispatchQueue.main.async {
                        self.isSending = false
                        self.lastSendStatus = .failure("HTTP \(httpResponse.statusCode): \(responseBody)")
                    }
